...
def main():
	...
	print 'Making a REST API call to DBpedia Spotlight for Concept Linking'
	...
	#datajson = content_file.read()
	#datajson=datajson.split('<HTML>')[0]
	#print datajson
	print 'Saved DBpedia Spotlight Response at data.json'
	...
	print 'Computed All Concept Similarity'

	print 'Generating Relationship Using OpenIE Tool'
	os.system('[ -e ./OpenIE/'+input_file+' ] && rm ./OpenIE/'+input_file)
	os.system('cp '+ input_file + ' ./OpenIE/')
	os.chdir ('OpenIE')
	os.system('python main.py -f '+input_file+' > output.json')
	os.system('[ -e ../output.json ] && rm ../output.json')
	os.system('cp output.json ../')
	os.chdir('..')
	openIEList=None
	print 'Saved OpenIE response at output.json'
	...
	print 'Saved Concept Map at output_image.png' 
